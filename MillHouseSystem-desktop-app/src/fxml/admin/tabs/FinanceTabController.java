...
public class FinanceTabController {
    ...
    public void initialize() {
        ...
        // Setup table columns - THIS IS CRITICAL
        setupTableColumns();
    ...
    private void setupTableColumns() {
        // Get the columns (they're defined in FXML)
        TableColumn<TransactionRow, String> dateColumn = (TableColumn<TransactionRow, String>) financialTable.getColumns().get(0);
        TableColumn<TransactionRow, Integer> countColumn = (TableColumn<TransactionRow, Integer>) financialTable.getColumns().get(1);
        TableColumn<TransactionRow, Double> revenueColumn = (TableColumn<TransactionRow, Double>) financialTable.getColumns().get(2);
        TableColumn<TransactionRow, Double> averageColumn = (TableColumn<TransactionRow, Double>) financialTable.getColumns().get(3);
        ...
        averageColumn.setCellValueFactory(cellData -> 
            new SimpleDoubleProperty(cellData.getValue().getAverage()).asObject());
        
        // Optional: Format the currency columns
        revenueColumn.setCellFactory(column -> new TableCell<TransactionRow, Double>() {
            @Override
        ...
        averageColumn.setCellFactory(column -> new TableCell<TransactionRow, Double>() {
            @Override
            protected void updateItem(Double value, boolean empty) {
                super.updateItem(value, empty);
                if (empty || value == null) {
