...
public class DeliveryManagementTabController {
 ...
 // IN: fxml/driver/tabs/DeliveryManagementTabController.java
    private void updateDeliveryStatusInDatabase(int orderId, String newStatus, String driverNotes) {
        ...
        try {
            ...
            String updateOrderSql = "UPDATE orders SET status = ?, driver_notes = ?, status_updated_at = NOW() WHERE id = ?";
            try (PreparedStatement ps = conn.prepareStatement(updateOrderSql)) {
                ...
                if (updated > 0) {
                    // 2. If status is "Delivered", update driver to "Available"
                    if ("Delivered".equals(newStatus)) {
                        updateDriverStatusToAvailable(conn);
                        
                        // 3. Update delivery logs
                        updateDeliveryLogs(conn, orderId);
                        
                        // 4. Sync to transactions
                        syncToTransactions(conn, orderId);
                    }
                    ...
                    // 6. Refresh local table
                    loadAssignedDeliveries();
                    
                    showAlert("Success", 
                             "Status updated to: " + newStatus + "\n" +
                             ( "Delivered".equals(newStatus) ? "Driver status set to Available\n" : "") +
    ...
    // NEW METHOD: Update driver status to Available
    private void updateDriverStatusToAvailable(Connection conn) throws SQLException {
        String driverName = CurrentUser.getInstance().getStaffName();
        if (driverName == null) return;
        
        // Check if driver has any other "On Route" or "Assigned to Driver" orders
        String checkOtherDeliveriesSql = "SELECT COUNT(*) as active_count FROM orders " +
                                        "WHERE assigned_driver LIKE ? " +
                                        "AND status IN ('Assigned to Driver', 'On Route', 'Ready for Delivery')";
        
        try (PreparedStatement checkPs = conn.prepareStatement(checkOtherDeliveriesSql)) {
            ...
            if (rs.next() && rs.getInt("active_count") == 0) {
                ...
                String updateDriverSql = "UPDATE drivers SET status = 'Available' WHERE name LIKE ?";
                try (PreparedStatement updatePs = conn.prepareStatement(updateDriverSql)) {
                    updatePs.setString(1, "%" + driverName + "%");
                    updatePs.executeUpdate();
                    System.out.println("Driver " + driverName + " status updated to Available");
                }
            } else {
                System.out.println("Driver " + driverName + " still has active deliveries, keeping current status");
            }
        }
    }

    // Update existing methods to use connection parameter
    private void updateDeliveryLogs(Connection conn, int orderId) throws SQLException {
    ...
    private void syncToTransactions(Connection conn, int orderId) throws SQLException {
