...
public class SettingsTabController {
    ...
    private void loadPrices() {
        ...
        try (Connection c = DatabaseConnection.getConnection();
             ...
             ResultSet rs = s.executeQuery("SELECT grain_type, price_per_kg FROM settings")) {
            while (rs.next()) {
                prices.add(new PriceSetting(rs.getString(1), rs.getDouble(2)));
            }
    ...
    private void loadThreshold() {
        try (Connection c = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                lowStockThresholdField.setText(String.valueOf(rs.getDouble(1)));
            } else {
